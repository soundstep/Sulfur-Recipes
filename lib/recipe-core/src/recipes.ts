...
export const HARDCODED_CATEGORIES: Record<string, string[]> = {
...
export function parseRecipeRows(wikitext: string): { variants: string[][]; categoryMap: Record<string, string>; specificMembers: Record<string, string[]> } {
...
export function detectType(wikitext: string): string {
...
export function resolveCategoryMembers(catName: string): string[] {
  const key = catName.toLowerCase();
  return HARDCODED_CATEGORIES[key] ?? [];
}

export async function fetchRecipeFromApi(slug: string): Promise<{ recipe: Recipe | null; categoryMap: Record<string, string>; specificMembers: Record<string, string[]> }> {
...
export async function getRecipesData(cache: CacheStore): Promise<RecipeCache> {
  ...
  const data: RecipeCache = { recipes: results, categoryMembers };
